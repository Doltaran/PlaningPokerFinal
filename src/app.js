(async () => {
 ...
 // Если URL-адрес не содержит идентификатора сеанса - сгенерируйте новый, затем перенаправьте.
  ...
  const App = {
    ...
    mounted() {
      ...
      // Установка значения карты по умолчанию без голосования.
  ...
  this.socket = new WebSocket(
    `${schema}://${window.location.hostname}:${window.location.port}${window.location.pathname}`
        ...
        this.socket.addEventListener("message", (messageEvent) => {
          ...
          if (message.type === MESSAGE_TYPE.STATE_REQUEST) {
            this.socket.send(
              JSON.stringify({
                ...
                playerId: this.playerId,
                playerName: this.playerName + '(' + this.playerRole + ')',
                playerRole: this.playerRole,
                vote: this.vote,
          ...
          // Если игрок отключен - уберите его карту со стола.
          ...
          // Если получено новое СОСТОЯНИЕ какого-либо игрока, просто обновите карты
          ...
          if (message.type === MESSAGE_TYPE.STATE) {
            ...
            this.cards.forEach((card) => {
              if (card.playerId === message.playerId) {
                ...
                card.playerRole = message.playerRole;
          ...
          // Если получено сообщение о раскрытии, откройте карточки после OPEN_DELAY
          if (message.type === MESSAGE_TYPE.REVEAL_CARDS) {
            // Если мы получим REVEAL_CARDS, запустим открытый интервал с самого начала.
            ...
            this.openDelayInterval = setInterval(() => {
              ...
              if (this.openDelayCounter <= 0) {
                ...
                // Обновите результаты голосования и рассчитайте среднее значение.
  ...
  /**
   * Генерируйте случайные варианты карточек.
   *
  ...
  // Требуется для правильной инициализации шаблона Vue.
  setTimeout(() => {
